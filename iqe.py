...
# iqebestfitx=np.array([])
# iqebestfity=np.array([])
# iqetxtfile = open('iqe/iqe.txt')
# for line in iqetxtfile:
#     iqebestfitx = np.append(iqebestfitx,[float(line.split()[0])])
#     iqebestfity = np.append(iqebestfity,[float(line.split()[1])])
# print(iqebestfitx)
# print(iqebestfity)
from pylab import *
ax1 = subplot(111)
ax1.scatter(wavelength,ginverse_tikhonov,marker='o',label='tkhonov',color='black')
ax1.scatter(wavelength,ginverse_tsvd,marker='+',label='tsvd',color='red')
